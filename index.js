...
client.on('interactionCreate', async (interaction) => {
    ...
    if (interaction.isStringSelectMenu()) {
        if (interaction.customId === 'LocationMovementSelector' + interaction.member.id) {
            ...
            var locations = await connection.promise().query('select ml.*, c.name as character_name from movement_locations ml left outer join characters c on c.location_id = ml.id join players_characters pc on pc.character_id = c.id join players p on p.id = pc.player_id where ((p.user_id = ? and pc.active = 1) or c.location_id = ?) and ml.movement_allowed = 1 and ml.guild_id = ?', [interaction.user.id, dest_id, interaction.guild_id]);
