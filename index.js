...
client.on('interactionCreate', async (interaction) => {
    if (interaction.isCommand()) { // TODO: && command_enabled(interaction, guild_id)
        ...
        if (interaction.commandName == 'addlocation') {
            ...
            if (interaction.options.getSubcommand() == 'add') {
                ...
                if (items[0].length > 0) {
                    if (items[0].length <= 25) {
                        ...
                        let itemsKeyValues = [];
                        for (const item of items[0]) {
                            let thisItemKeyValue = { label: item.name, value: item.id.toString() };
                        ...
                        let itemsKeyValues = [];
                        for (const item of items) {
                            let thisItemKeyValue = { label: item, value: item }
                            itemsKeyValues.push(thisItemKeyValue);
                        }
                    ...
                    let message = await interaction.reply({ content: 'Please select an item to add an effect to:', components: [itemSelectRow], ephemeral: true });
                    let collector = message.createMessageComponentCollector();
                    let selectedItem;
                    let type;
                    let visible;
                        ...
                        if (interaction_second.member.id === interaction.member.id) {
                            if (interaction_second.customId == 'ItemEffectAlphabetSelector') {
                                ...
                                let targets = [
                                    ...
                                    { label: 'Item Target', value: 'target' }
                                ];
                                let selectComponent = new StringSelectMenuBuilder().setOptions(targets).setCustomId('TargetSelector').setMinValues(1).setMaxValues(1);
                                let selectRow = new ActionRowBuilder().addComponents(selectComponent);
                                await interaction_second.update({ content: 'Please select a target for this effect:', components: [selectRow] });
            ...
            if (interaction.member.permissions.has('ADMINISTRATOR')) {
                if (character) {
                    var characters = await connection.promise().query('select * from characters where guild_id = ? and name like ?', [interaction.guildId, '%' + character + '%']);
                } else {
                    var characters = await connection.promise().query('select * from characters where guild_id = ?', [interaction.guildId]);
                }
            } else {
                if (character) {
                    var characters = await connection.promise().query('select c.* from players p join players_characters pc on p.id = pc.player_id join characters c on pc.character_id = c.id where p.user_id = ? and p.guild_id = ? and c.name like ?', [interaction.user.id, interaction.guildId, '%' + character + '%']);
                } else {
                    var characters = await connection.promise().query('select c.* from players p join players_characters pc on p.id = pc.player_id join characters c on pc.character_id = c.id where p.user_id = ? and p.guild_id = ?', [interaction.user.id, interaction.guildId]);
                }
                        ...
                        if (skills[0].length > 0) {
                            ...
                            collector.on('collect', async (interaction_second) => {
                                if (interaction_second.member.id === interaction.member.id) {
                                    if (interaction_second.values[0]) {
                                        skillSelected = interaction_second.values[0];
                                        let skill = skills.find(s => s.id == skillSelected);
