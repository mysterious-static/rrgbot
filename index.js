    ...
    if (interaction.isCommand()) { // TODO: && command_enabled(interaction, guild_id)
            ...
            if (archetypes[0].length > 0) {
                ...
                collector.on('collect', async (interaction_second) => {
                    if (interaction_second.customId == 'ArchetypeAssignmentSelector') {
                        ...
                        var characters = await connection.promise().query('select characters.* from characters left outer join characters_archetypes ca on characters.id = ca.character_id where guild_id = ? and (ca.archetype_id <> ? or ca.archetype_id is null)', [interaction.guildId, selectedArchetype]);
                        if (characters[0].length > 0) {
        ...
        // PLAYER COMMANDS
            ...
            if (interaction.commandName == 'move') {
                ...
                var current_character = await connection.promise().query('select character_id from players_characters join players p on p.id = players_characters.player_id where p.user_id = ? and players_characters.active = 1', [interaction.user.id]);
                var archetypeskills = await connection.promise().query('select s.* from skills s join skills_archetypes sa on sa.skill_id = s.id join characters_archetypes ca on sa.archetype_id = ca.archetype_id where ca.character_id = ?', [current_character[0].character_id]);
                var characterskills = await connection.promise().query('select s.* from skills s join skills_characters sc on sc.skill_id = s.id where sc.character_id = ?', [current_character[0].character_id]);
                var skills;
                ...
                var current_character = await connection.promise().query('select c.location, pc.character_id from players_characters pc join characters c on c.id = pc.character_id join players p on p.id = players_characters.player_id where p.user_id = ? and players_characters.active = 1', [interaction.user.id]);
                var items = await connection.promise().query('select i.* from items i join characters_items ci on ci.item_id = i.id where ci.character_id = ?', [current_character[0].id]);
                if (items[0].length > 0) {
                    var itemsKeyValues = [];
                    for (const item of items[0]) {
