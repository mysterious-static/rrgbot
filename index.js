...
const { ChannelType, ModalBuilder, TextInputBuilder, ActionRowBuilder, ButtonBuilder, EmbedBuilder, TextInputComponent, StringSelectMenuBuilder, RoleSelectMenuBuilder, TextInputStyle, PermissionFlagsBits, PermissionsBitField, GatewayIntentBits, SlashCommandBuilder, ButtonStyle, MessageFlags } = require('discord.js')
...
client.on('interactionCreate', async (interaction) => {
                            ...
                            if (interaction_second.member.id === interaction.member.id) {
                                if (interaction_second.values[0]) {
                                    ...
                                    if (locationSelected && characterSelected) {
                                        let character = await connection.promise().query('select c.*, p.user_id from characters c join players_characters pc on c.id = pc.character_id join players p on p.id = pc.player_id where c.id = ? and c.guild_id = ?', [characterSelected, interaction.guildId]);
                    ...
                    if (interaction_second.member.id === interaction.member.id) {
                        if (interaction_second.values[0]) {
                            if (interaction_second.customId === 'ArchetypeStatAssignmentStatSelector') {
                                ...
                                let archetype_ids = [];
                                for (const thisArchetype of archetype[0]) {
    ...
    if (interaction.isStringSelectMenu()) {
        if (interaction.customId === 'LocationMovementSelector' + interaction.member.id) {
            ...
            let locations = await connection.promise().query('select distinct ml.* from movement_locations ml left outer join characters c on c.location_id = ml.id left outer join players_characters pc on pc.character_id = c.id left outer join players p on p.id = pc.player_id where ((p.user_id = ? and pc.active = 1 and c.guild_id = ?) or ml.id = ?) and ml.movement_allowed = 1 and ml.guild_id = ?', [interaction.member.id, interaction.guildId, dest_id, interaction.guildId]); //todo get dest id working
            let active_character = await connection.promise().query('select c.* from characters c join players_characters pc on pc.character_id = c.id join players p on p.id = pc.player_id where p.user_id = ? and pc.active = 1 and c.guild_id = ?', [interaction.member.id, interaction.guildId]);
