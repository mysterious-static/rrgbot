...
const { Permissions, MessageActionRow, TextInputComponent, MessageButton, MessageSelectMenu, TextInputStyle, Modal, PermissionsFlagsBits } = require('discord.js')
const client = new Discord.Client({ intents: ["GUILDS", "GUILD_MESSAGES", "GUILD_EMOJIS_AND_STICKERS", "GUILD_WEBHOOKS", "GUILD_MEMBERS"] });
...
if (potential_command == '!link') {
    var age_name = message.content.substr(message.content.indexOf(' ') + 1).replace(/[\u2018\u2019]/g, "'").toLowerCase();
    var queryData = [age_name, message.author.id];
    var response = await connection.promise().query('select * from ages where lower(name) = ? and movement_allowed = 1; select players.*, ages.mechanics_channel_id, ages.channel_id, ages.hide_on_leave, ages.movement_allowed_out from players join ages on ages.id = players.age where discord_uid = ?', queryData);
    if (response[0][0].length > 0) {
        var age = response[0][0][0];
        var player = response[0][1][0];
        var old_channel = await client.channels.cache.get(player.channel_id);
        if (player.movement_allowed_out) {
            if (player.hide_on_leave) {
                await old_channel.send('*' + player.name + ' links out!*');
                await old_channel.permissionOverwrites.edit(message.author, { VIEW_CHANNEL: false, READ_MESSAGE_HISTORY: false, SEND_MESSAGES: false });
                await client.channels.cache.get(player.mechanics_channel_id).permissionOverwrites.edit(message.author, { VIEW_CHANNEL: false, READ_MESSAGE_HISTORY: false, SEND_MESSAGES: false });
            } else {
                await old_channel.send('*' + player.name + ' links out!*')
                await old_channel.permissionOverwrites.edit(message.author, { VIEW_CHANNEL: true, READ_MESSAGE_HISTORY: true, SEND_MESSAGES: false, SEND_MESSAGES_IN_THREADS: true });
                await client.channels.cache.get(player.mechanics_channel_id).permissionOverwrites.edit(message.author, { VIEW_CHANNEL: true, READ_MESSAGE_HISTORY: true, SEND_MESSAGES: false, SEND_MESSAGES_IN_THREADS: true });
            }
            var new_channel = client.channels.cache.get(age.channel_id);
            await new_channel.permissionOverwrites.edit(message.author, { VIEW_CHANNEL: true, READ_MESSAGE_HISTORY: true, SEND_MESSAGES: true, SEND_MESSAGES_IN_THREADS: true });
            new_channel.send('*' + player.name + ' links in!*');
            await client.channels.cache.get(age.mechanics_channel_id).permissionOverwrites.edit(message.author, { VIEW_CHANNEL: true, READ_MESSAGE_HISTORY: true, SEND_MESSAGES: true, SEND_MESSAGES_IN_THREADS: true });
            queryData = [age.id, player.id];
            await connection.promise().query('update players set age = ? where id = ?', queryData);
            message.react('✅');
            if (age.id != player.starting_age && !age.free_movement && age.id != player.second_age) {
                queryData = [age.id, message.author.id];
                await connection.promise().query('update players set has_linked = 1, second_age = ? where discord_uid = ?', queryData);
            }
        } else {
            message.reply('You can\'t link out of this Age.');
        }
    } else {
        message.reply('You didn\'t provide a valid age to link to.');
    }
}