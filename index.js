...
client.on('interactionCreate', async (interaction) => {
    ...
    if (interaction.isStringSelectMenu()) {
        if (interaction.customId === 'LocationMovementSelector' + interaction.member.id) {
            ...
            console.log(interaction.guildId);
            var locations = await connection.promise().query('select ml.*, c.name as character_name from movement_locations ml left outer join characters c on c.location_id = ml.id left outer join players_characters pc on pc.character_id = c.id left outer join players p on p.id = pc.player_id where (p.user_id = ? and pc.active = 1) and ml.movement_allowed = 1 and ml.guild_id = ?', [interaction.member.id, interaction.guild_id]); //todo get dest id working
            console.log(locations[0]);
