        ...
        // PLAYER COMMANDS
            ...
            if (interaction.commandName == 'move') {
                ...
                if (isHealthStat[0].length > 0) {
                    ...
                    var player = await connection.promise().query('select c.* from characters c join players_characters pc on characters.id = pc.character_id join players p on pc.player_id = p.id where pc.active = 1 and p.user_id = ?', [interaction.user.id]);
                    var target = await connection.promise().query('select c.* from characters c join players_characters pc on characters.id = pc.character_id join players p on pc.player_id = p.id where pc.active = 1 and p.user_id = ?', [target.id]);
