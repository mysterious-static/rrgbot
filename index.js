...
client.on('interactionCreate', async (interaction) => {
    if (interaction.isCommand()) { // TODO: && command_enabled(interaction, guild_id)
        ...
        if (interaction.commandName === 'addlocation') {
            ...
            if (interaction.options.getSubcommand() === 'charactersummary') {
                ...
                if (reputations[0].length > 0) {
                    if (reputations[0].length == 1) {
                        let reputations_tiers_characters = await connection.promise().query('select r.*, rt.threshold_name, c.name, cr.value as rep_value as character_name from reputations r join characters_reputations cr on r.id = cr.reputation_id join reputations_tiers rt on r.id = rt.reputation_id join characters c on cr.character_id = c.id where r.guild_id = ? having MAX(rt.value)<= cr.value', [interaction.guildId]);
                        let message = '';
            ...
            if (interaction.options.getSubcommand() === 'add') {
                ...
                const reputationSelectRow = new ActionRowBuilder().addComponents(reputationSelectComponent);
                let message = await interaction.reply({ content: 'Please select a reputation:', components: [reputationSelectRow], ephemeral: true });
                let collector = message.createMessageComponentCollector();
                let type;
                let visible;
                let tier_id;
    ...
    if (interaction.isButton()) {
        if (interaction.customId === 'rpsButtonR' || interaction.customId === 'rpsButtonP' || interaction.customId === 'rpsButtonS') {
            ...
            let character_reputations = await connection.promise().query(`select r.*, cr.value as characterStanding from reputations r join characters_reputations cr on r.id = cr.reputation_id left outer join characters_characterflags cc on (r.visibility = "cflag" and r.cwflag_id = cc.characterflag_id and cc.character_id = ?) left outer join worldflags w on (r.visibility = "wflag" and r.cwflag_id = w.id) where cr.character_id = ? and (r.visibility = "always" or (r.visibility = "cflag" and cc.value is not null and cc.value >= r.cwflag_value) or (r.visibility = "wflag" and w.value is not null and w.value >= r.cwflag_value)) order by r.id ${sort}`, [character_id, character_id]); // Filter this by cflag visibility
            let msg = '';
            let firstDisplayedId = false;
            let lastDisplayedId = false;
            let maxId = false;
