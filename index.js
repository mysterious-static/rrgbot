                        ...
                        collector.on('collect', async (interaction_second) => {
                            if (interaction_second.member.id === interaction.member.id) {
                                if (interaction_second.customId === 'EffectViewRepSelector') {
                                    ...
                                    let effects = await connection.promise().query('select count(ifnull(ep.id, 0)) as prereq_count, e.* from effects e join reputations_tiers_effects rte on e.id = rte.effect_id left outer join effects_prereqs ep on e.id = ep.effect_id where rte.reputationtier_id = ? group by ep.id', [interaction_second.values[0]]);
                                    let embed = new EmbedBuilder()
